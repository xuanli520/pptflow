...
func addRunnerFlags(cmd *cobra.Command, opts *app.RunnerOptions) {
	...
	cmd.Flags().IntVar(&opts.HarborConcurrency, "harbor-concurrency", domain.DefaultHarborConcurrency, "Harbor concurrent trials per model (1-4 for pass@4; runs in bounded batches)")
	cmd.Flags().IntVar(&opts.HarborAttempts, "harbor-attempts", domain.RequiredTrialCount, "Harbor trial attempts per model (must remain 4 for pass@4)")
