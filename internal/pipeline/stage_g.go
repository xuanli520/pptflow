...
func (r Runner) stageG(ctx context.Context, sc StageContext) model.StageRecord {
	...
	profile, err := os.ReadFile(filepath.Join(r.cfg.Codex.PromptProfilesDir, "frontend_e2e_browser.md"))
	if err != nil {
		record.Findings = append(record.Findings, model.Finding{
			Stage:      string(model.StageG),
			Severity:   "High",
			Title:      "Stage G prompt profile missing",
			Rule:       "Stage G requires the frontend_e2e_browser.md prompt profile.",
			Evidence:   err.Error(),
			Impact:     "Codex browser planning cannot start.",
			MinimumFix: "Ensure assets were released and rerun Stage G.",
			SourcePath: filepath.Join(r.cfg.Codex.PromptProfilesDir, "frontend_e2e_browser.md"),
		})
	...
	for round := 1; round <= stageGMaxActions; round++ {
		...
		rawAction, warnings, err := r.nextBrowserAction(stageCtx, sc, string(profile), contextText, candidates, observations, blocked, round, turnTimeout)
		...
		if validation.Action.Action == "finish" {
			...
			observationFindings := frontendE2EObservationFindings(observations, writer.RelativePath(screenshotPath))
...
func (r Runner) finishStageGUnavailable(record model.StageRecord, writer ArtifactWriter, start time.Time, reason, status string, summary FrontendE2ESummary, observations []browserpkg.Observation) model.StageRecord {
	...
	record = r.writeStageGArtifacts(record, writer, summary, observations)
	bestEffortStageText(&record, writer, writer.RelativePath(record.LogPath), reason)
	return finishStage(record, status, start)
}
...
func frontendE2EObservationFindings(observations []browserpkg.Observation, screenshot string) []model.Finding {
