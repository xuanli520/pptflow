...
// composition before it is persisted with the Run.
...
// before it has durably committed the deployment profile/catalog that will
// govern any retry.
...
func (service *StandardAuthoringLaunchService) Start(ctx context.Context, command StandardAuthoringLaunchCommand) (LifecycleMutationReceipt, error) {
	...
	session, err := service.core.store.CreateAuthoringSession(ctx, store.CreateAuthoringSessionRequest{
		...
		SourceID:                source.ID,
		TargetTaskID:            task.ID,
		WorkflowTemplateID:      frozen.Profile.Template.ID,
		WorkflowTemplateVersion: frozen.Profile.Template.Version,
...
func (service *StandardAuthoringLaunchService) freezeStandardAuthoringLaunchDeploymentDefinition(ctx context.Context) (standardAuthoringLaunchDeploymentDefinition, error) {
	...
	return service.newStandardAuthoringLaunchDeploymentDefinition(definition.Profile, definition.DeploymentCatalogReceipt)
}

func (service *StandardAuthoringLaunchService) newStandardAuthoringLaunchDeploymentDefinition(profile workflowadapter.ExecutionProfile, receipt []byte) (standardAuthoringLaunchDeploymentDefinition, error) {
	...
	catalogReceipt, err := service.core.resolveStartRunDeploymentCatalogReceipt(profile.Template, receipt)
	if err != nil {
	...
	fingerprint, err := standardAuthoringStaticDefinitionFingerprint(profileCanonical, catalogReceipt)
	if err != nil {
...
func standardAuthoringStaticDefinitionFingerprint(profile, receipt []byte) (workflowkit.Fingerprint, error) {
...
func (preparation standardAuthoringLaunchPreparation) DeploymentDefinition() (standardAuthoringLaunchDeploymentDefinition, error) {
	if preparation.Format != standardAuthoringLaunchPreparationFormat || preparation.Version != standardAuthoringLaunchPreparationVersion ||
		preparation.WorkflowTemplateID != workflowadapter.StandardAuthoringWorkflowTemplateID || !workflowadapter.StandardAuthoringCurrentTemplateReference().Equal(workflowadapter.TemplateReference{ID: preparation.WorkflowTemplateID, Version: preparation.WorkflowTemplateVersion}) ||
	...
	definition, err := newStandardAuthoringLaunchDeploymentDefinitionWithoutResolver(profile, preparation.DeploymentCatalogReceipt)
	if err != nil {
		return standardAuthoringLaunchDeploymentDefinition{}, err
	}
	...
	if !bytes.Equal(preparation.ExecutionProfile, definition.ProfileCanonical) || !bytes.Equal(preparation.DeploymentCatalogReceipt, definition.DeploymentCatalogReceipt) ||
		preparation.ProfileFingerprint != definition.ProfileFingerprint || preparation.PreparationFingerprint != preparationFingerprint {
...
func newStandardAuthoringLaunchDeploymentDefinitionWithoutResolver(profile workflowadapter.ExecutionProfile, receipt []byte) (standardAuthoringLaunchDeploymentDefinition, error) {
	if !profile.Template.Equal(workflowadapter.StandardAuthoringCurrentTemplateReference()) {
	...
	if err != nil || !parsed.Template.Equal(profile.Template) || !bytes.Equal(receipt, canonical) {
...
func (service *StandardAuthoringLaunchService) freezeStandardAuthoringDefinition(ctx context.Context, subject StandardAuthoringRunDefinitionSubject, expected standardAuthoringLaunchPreparation, contract standardAuthoringContractInput) (standardAuthoringFrozenDefinition, error) {
	...
	if !definition.Profile.Template.Equal(workflowadapter.StandardAuthoringCurrentTemplateReference()) || !definition.ExecutionSpec.Template.Equal(definition.Profile.Template) || !definition.Profile.Template.Equal(expectedDefinition.Profile.Template) {
	...
	currentStaticDefinition, err := service.newStandardAuthoringLaunchDeploymentDefinition(definition.Profile, catalogReceipt)
	if err != nil {
	...
	definitionFingerprint, err := standardAuthoringDefinitionFingerprint(profileCanonical, specificationCanonical, catalogReceipt)
	if err != nil {
...
func standardAuthoringDefinitionFingerprint(profile, specification, receipt []byte) (workflowkit.Fingerprint, error) {
...
// a registered harbor.standard-authoring template and has no caller-selectable operation,
...
func NewCatalogStandardAuthoringRunDefinitionProvider(catalog *stageprovider.DeploymentOperationCatalogResolver, profile workflowadapter.ExecutionProfile) (*CatalogStandardAuthoringRunDefinitionProvider, error) {
	if catalog == nil || !catalog.Template().Equal(workflowadapter.StandardAuthoringCurrentTemplateReference()) {
		return nil, fmt.Errorf("Standard authoring deployment catalog is required")
	}
	if !profile.Template.Equal(workflowadapter.StandardAuthoringCurrentTemplateReference()) || !profile.Template.Equal(catalog.Template()) {
...
// the template-scoped catalog receipt around this profile before it permits a
// Git remote contact.
func (provider *CatalogStandardAuthoringRunDefinitionProvider) StandardAuthoringStaticRunDefinition(context.Context) (StandardAuthoringStaticRunDefinition, error) {
	if provider == nil || provider.catalog == nil || !provider.catalog.Template().Equal(workflowadapter.StandardAuthoringCurrentTemplateReference()) {
...
func standardAuthoringCatalogStageBinding(registration stageprovider.DeploymentOperationRegistration, secretIDs []string) (workflowadapter.StageExecutionBinding, error) {
	...
	case workflowadapter.StageBindingTestsAnalysis:
		return workflowadapter.UniversalStageBinding{StageBindingBase: base}, nil
	case workflowadapter.StageBindingCodeEdgePackageAdmission:
		return workflowadapter.UniversalStageBinding{StageBindingBase: base}, nil
