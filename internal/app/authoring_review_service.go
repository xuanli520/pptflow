...
func (service *AuthoringReviewService) Inspect(ctx context.Context, reviewRequestID string) (AuthoringReviewGateSnapshot, error) {
	...
	if !isCurrentStandardAuthoringRun(*run) {
		return AuthoringReviewGateSnapshot{}, fmt.Errorf("authoring review Run is not %s@%s", workflowadapter.StandardAuthoringWorkflowTemplateID, workflowadapter.StandardAuthoringBriefTemplateVersion)
	}
