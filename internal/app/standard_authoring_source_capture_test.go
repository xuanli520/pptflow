...
func TestStandardAuthoringGitArchiveSourceCapturerExecutesControlledHTTPSCapture(t *testing.T) {
	...
	script := "#!/bin/sh\n" +
		...
		"      printf 'git_http_low_speed_limit_key=%s\\n' \"${GIT_CONFIG_KEY_0-}\"\n" +
		"      printf 'git_http_low_speed_limit=%s\\n' \"${GIT_CONFIG_VALUE_0-}\"\n" +
		"      printf 'git_http_low_speed_time_key=%s\\n' \"${GIT_CONFIG_KEY_1-}\"\n" +
	...
	for _, expected := range []string{
		"git_allow_protocol=https:ssh", "git_protocol_from_user=0", "git_config_global=/dev/null", "git_config_count=2", "git_http_low_speed_limit_key=http.lowSpeedLimit", "git_http_low_speed_limit=1", "git_http_low_speed_time_key=http.lowSpeedTime", "git_http_low_speed_time=45", "git_terminal_prompt=0", "ssh_auth_sock=unset",
	} {
