...
// contract emitted by task_synthesis and frozen by task_review. It belongs in
// workflowadapter because both the author submission boundary and host verifier
// must accept exactly the same canonical bytes.
...
func ParseStandardAuthoringVerificationContractJSON(raw []byte) (StandardAuthoringVerificationContract, error) {
	...
	if err := decodeExecutionSpecJSON(raw, &contract); err != nil {
		return StandardAuthoringVerificationContract{}, fmt.Errorf("Standard authoring verification contract: %w", err)
	}
