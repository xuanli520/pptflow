...
func TestStandardAuthoringHarnessTemplateBindsValidatedArtifacts(t *testing.T) {
	...
	if err := template.Validate(); err != nil {
		t.Fatalf("validate v2 template: %v", err)
	}
	if !template.Reference().Equal(StandardAuthoringCurrentTemplateReference()) ||
		template.Catalog.Version != StandardAuthoringContractTemplateVersion ||
		template.QuotaPolicy.Version != StandardAuthoringContractQuotaPolicyVersion {
	...
	if err != nil || !reflect.DeepEqual(order, StandardAuthoringStageOrder()) || len(order) != len(template.Catalog.Stages) {
		t.Fatalf("v2 stage order = %v catalog=%d err=%v", order, len(template.Catalog.Stages), err)
	}
	if handoff, err := StandardAuthoringTaskHandoffSchemaForTemplate(template.Reference()); err != nil || handoff != StandardAuthoringTaskHandoffSchemaVersion {
		t.Fatalf("v2 handoff schema = %q, %v", handoff, err)
	}
	...
	for _, contract := range []struct {
		...
		{stage: SolutionReview, artifact: "task_toml", resource: resourceTaskMetadata},
		{stage: SolutionReview, artifact: StandardAuthoringValidatedDockerfileArtifact, resource: resourceTaskValidatedEnvironment},
...
func TestStandardAuthoringContractTemplateIsCurrentAndBindsEveryStage(t *testing.T) {
	template := StandardAuthoringContractWorkflowTemplate()
	if err := template.Validate(); err != nil {
		t.Fatalf("validate v2 template: %v", err)
	}
	if !template.Reference().Equal(StandardAuthoringContractTemplateReference()) ||
		...
		template.Catalog.Version != StandardAuthoringContractTemplateVersion ||
		template.QuotaPolicy.Version != StandardAuthoringContractQuotaPolicyVersion {
		t.Fatalf("v2 identities = template %s catalog %s quota %s", template.Version, template.Catalog.Version, template.QuotaPolicy.Version)
	}
	order, err := StandardAuthoringStageOrderForTemplate(template.Reference())
	if err != nil || !reflect.DeepEqual(order, StandardAuthoringStageOrder()) || len(order) != len(template.Catalog.Stages) {
		t.Fatalf("v2 stage order = %v catalog=%d err=%v", order, len(template.Catalog.Stages), err)
	...
	for _, stage := range template.Catalog.Stages {
		...
		if !present || !contract.Required || contract.SchemaVersion != AuthoringContractSchemaVersion {
			t.Fatalf("stage %q root contract = %+v", stage.Key, contract)
		}
		if count := stageCatalogResourceCount(stage.ReadSet, resourceAuthoringContract); count != 1 {
			t.Fatalf("stage %q root contract resource count = %d, want 1", stage.Key, count)
		}
		if _, present := artifactSpecNamed(stage.Inputs, "authoring_brief"); present {
			t.Fatalf("stage %q retains authoring_brief", stage.Key)
		}
		if _, present := artifactSpecNamed(stage.Inputs, "environment_policy"); present {
			t.Fatalf("stage %q retains environment_policy", stage.Key)
		}
	}
}
...
func TestStandardAuthoringHarnessQuotaSeparatelyBoundsValidation(t *testing.T) {
	...
	if err := policy.ValidateFor(StandardAuthoringCurrentWorkflowTemplate().Catalog); err != nil {
		t.Fatalf("validate v2 quota policy: %v", err)
	}
	...
	for _, key := range []workflowkit.StageKey{workflowkit.StageKey(DockerfileBuildValidate), workflowkit.StageKey(AuthoringHarness)} {
		...
		if !present || !hasQuotaClaim(claims, "stage_attempt", 1) || !hasQuotaClaim(claims, "agent_turn", 1) ||
			!hasQuotaClaim(claims, "output_submission", StandardAuthoringWorkspaceSubmissionClaimUnits) ||
			!hasQuotaClaim(claims, StandardAuthoringValidationQuotaDimension, StandardAuthoringValidationClaimUnits) {
			t.Fatalf("v2 workspace stage %q claims = %+v", key, claims)
		}
	}
...
func TestStandardAuthoringV2CatalogRejectsValidationDrift(t *testing.T) {
	testCases := []struct {
		name   string
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			if err := catalog.Validate(); err == nil {
				t.Fatal("drifted v2 catalog validated")
			}
		})
	}
}
...
func requireAuthoringHarnessStage(t *testing.T, catalog StageCatalog, key string) StageDefinition {
	...
	if !present {
		t.Fatalf("v2 catalog omits stage %q", key)
	}
	return stage
}
