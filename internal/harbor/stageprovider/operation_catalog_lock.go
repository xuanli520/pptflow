...
// an installed deployment lock. Runtime code resolves it against the currently
// deployed lock; it is never frozen into a Run manifest.
...
// VerifyLockIdentity proves a compact lock binding belongs to this resolver
// before a worker trusts the installed runtime configuration.
func (resolver *DeploymentOperationCatalogLockResolver) VerifyLockIdentity(identity DeploymentOperationCatalogLockIdentity) error {
	...
	if identity != resolver.LockIdentity() {
		return fmt.Errorf("%w: installed lock %q@%q fingerprint %q does not match loaded lock", ErrDeploymentOperationCatalogLockDrift, identity.LockID, identity.LockVersion, identity.Fingerprint)
	}
	return nil
}
...
// LockIdentity exposes the static immutable lock identity for runtime
// evidence binding alongside the frozen catalog receipt.
...
// VerifyLockIdentity forwards installed lock-identity verification without
// exposing the private static verifier. Lifecycle and worker code re-verify
// the currently deployed lock identity at every worker boundary.
